...
export class CouponDialogComponent implements OnInit {
  ...
  public onSubmit() {
    if (this.modoEditar) {
      ...
      this.AdminService.editarOferta(idProducto, tipoOferta, descuentoEnPorcentaje, fechaInicio, fechaFin, descripcion, condicionesOferta, codigoOferta, numeroUso, estatus).subscribe(result => {
      ...
      this.AdminService.crearOferta(idProducto, tipoOferta, descuentoEnPorcentaje, fechaInicio, fechaFin, descripcion, condicionesOferta, codigoOferta, numeroUso, estatus).subscribe(result => {
        console.log(result)
        Util.successMessage(result.mensaje);
        this.dialogRef.close(result);
        window.location.reload();
